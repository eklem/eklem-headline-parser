...
test('return single most relevant keyword from title', function (t) {
  t.plan(2)
  t.deepEqual(findKeywords(stubs[0].headline, stubs[0].body, stubs[0].n, stubs[0].args, stubs[0].returnNonMatched), ['African'])
  t.deepEqual(findKeywords(stubs[1].headline, stubs[1].body, 1, stubs[1].args, stubs[1].returnNonMatched), ['absolutely'])
})

test('return top three keywords sorted by relevance', function (t) {
  t.plan(1)
  t.deepEqual(findKeywords(stubs[1].headline, stubs[1].body, stubs[1].n, stubs[1].args, stubs[1].returnNonMatched), ['final','exclamation','absolutely'])
})

test('return all keywords when no N or args argument is given', function (t) {
  t.plan(1)
  t.deepEqual(findKeywords(stubs[1].headline, stubs[1].body, null, null, null), ['final','exclamation','absolutely'])
})
...
test('return empty array if headline has only stopwords', function (t) {
  t.plan(1)
  t.deepEqual(findKeywords(stubs[2].headline, stubs[2].body, stubs[2].n, stubs[2].args, stubs[2].returnNonMatched), [])
})

test('return all sorted keywords if N is greater than number of keywords', function (t) {
  t.plan(1)
  t.deepEqual(findKeywords(stubs[1].headline, stubs[1].body, 10, stubs[1].args, null), ['final','exclamation','absolutely'])
})

test('return all sorted keywords if no N is given', function (t) {
  t.plan(1)
  t.deepEqual(findKeywords(stubs[1].headline, stubs[1].body, null, stubs[1].args, null), ['final','exclamation','absolutely'])
})

test('return no keywords since returnNonMatched is set to to false', function (t) {
  t.plan(1)
  t.deepEqual(findKeywords(stubs[3].headline, stubs[3].body, stubs[3].n, stubs[3].args, stubs[3].returnNonMatched), [])
})

test('return no keywords since returnNonMatched defaults to false', function (t) {
  t.plan(1)
  t.deepEqual(findKeywords(stubs[3].headline, stubs[3].body, stubs[3].n, stubs[3].args, null), [])
})

test('return keywords even if no match since returnNonMatched is true', function (t) {
  t.plan(1)
  t.deepEqual(findKeywords(stubs[3].headline, stubs[3].body, stubs[3].n, stubs[3].args, true), ['imperative','otter','shenanigans'])
})
